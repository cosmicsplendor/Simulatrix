    ...
    static updateOffEdge(entity, dt) {
        ...
        switch (entity.offEdge) {
            case -1: // off left-edge
                var cond1 = (a1 > Math.PI / 2 ) // whether entity-center to block-corner angle is greater than 90 degrees, in which case the entity is no longer off edge
                if (cond1 || a1 < offLeftThreshold) { // if cond1 or if the angle is less than 45 degrees (in latter case the ball should go to free fall)  
                    entity.offEdge = 0
                    !cond1 && (entity.velX = -offEdgeToppleSpeed)
                    return cond1 && (entity.pos.y -= 1 / 2) // just a small decrement to ensure that the circle ends up in a non-colliding state
                } 
            break
